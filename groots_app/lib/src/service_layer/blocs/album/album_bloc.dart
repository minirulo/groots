...
class AlbumBloc extends Bloc<AlbumEvent, AlbumState> {
  ...
  AlbumBloc({required Messagebus bus}) : _bus = bus, super(const AlbumState()) {
  ...
  Future<void> _onCreate(
    AlbumCreateRequested event,
    Emitter<AlbumState> emit,
  ) async {
    try {
      final albumId = await _bus.handle<String>(
        CreateAlbumCommand(
          title: event.title,
          artist: event.artist,
          year: event.year,
  ...
  Future<void> _onAssign(
    AlbumTrackAssignRequested event,
    Emitter<AlbumState> emit,
  ) async {
    try {
      await _bus.handle(
  ...
  Future<void> _onUnassign(
    AlbumTrackUnassignRequested event,
    Emitter<AlbumState> emit,
  ) async {
    try {
      await _bus.handle(
